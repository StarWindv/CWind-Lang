...
语法与 Rust 同源:
 ...
 - 基础类型中, 只有 Tuple / Vector / Map / Set 支持泛型
...
| 复合类型声明                     | 主类型 + 尖括号对 + 内部类型 | <>               | 类似 Rust, 例如 Vector<String> 代表一个内部元素全为 String 的向量数组; 复合类型识别属 Parser 职责: `<`/`>` 及其组合在词法上只是普通记号, 泛型与比较/移位的区分、嵌套收尾 (`Vector<Vector<Int>>`) 的拆分由 Parser 在上下文内处理; 完整的泛型机制 (自定义结构体 / impl 泛型) 当前暂不加入, 见 1.1 |
| 表达作用域                       | 在特定关键字后使用花括号对   | {}               |                                                                                                                                                                                                                                                                                                 |
