...
| 不小于                           | 叹号左尖括号                 | !<               | 自动转换为 >=                                                                                                                                                                                                                   |
| 不大于                           | 叹号右尖括号                 | !>               | 自动转换为 <=                                                                                                                                                                                                                   |
| 转义                             | 单反斜杠                     | \                | 特殊转义字符: `\n` `\r` `\t` `\\` `\'` `\"` `\0` `\b` `\v`; `\` 后紧跟换行表示转义换行, 缩进计入字符串; 字符串外将紧随其后的符号按字面处理                                                                                      |
...
| 取余                             | 单百分号                     | %                |                                                                                                                                                                                                                                 |
| 乘法                             | 单星号                       | *                |                                                                                                                                                                                                                                 |
| 加法                             | 单正号                       | +                | 用于数字求和, 字符串与任意类型求和; 字符串加法在 rt 中通过 trait 实现                                                                                                                                                           |
| 左移                             | 双左尖括号                   | <<               |                                                                                                                                                                                                                                 |
| 右移                             | 双右尖括号                   | \>\>             |                                                                                                                                                                                                                                 |
| 按位与                           | 单and符号                    | &                |                                                                                                                                                                                                                                 |
| 按位或                           | 单管道符号                   | \|               |                                                                                                                                                                                                                                 |
...
| 解包列表或map                    | 两个英文句点后跟待解包对象   | ..               | Vector 解包后得到 value 串, 每个元素之间以逗号分隔; Map 解包后得到元组串 (key, value), 每个kv之间以逗号分隔; 解包只能出现在方法调用的参数中, 且标识符不能以数字或符号开头, 故 `..` 不会与 `.` 属性/方法访问混淆                 |
| 定义参数列表/条件表达式          | 圆括号对                     | ()               |                                                                                                                                                                                                                                 |
| 切片索引/定义数组                | 方括号对                     | []               | 类似 Python, 切片中的 `:`/`::` 由上下文区分                                                                                                                                                                                     |
| Map 中取键                       | 方括号对 + 键名              | []               | 类似 Python                                                                                                                                                                                                                     |
| 复合类型声明                     | 主类型 + 尖括号对 + 内部类型 | <>               | 类似 Rust, 例如 Vector<String> 代表一个内部元素全为 String 的向量数组; 复合类型识别属 Parser 职责: `<`/`>` 及其组合在词法上只是普通记号, 泛型与比较/移位的区分、嵌套收尾 (`Vector<Vector<Int>>`) 的拆分由 Parser 在上下文内处理 |
| 表达作用域                       | 在特定关键字后使用花括号对   | {}               |                                                                                                                                                                                                                                 |
...
### 1.3 关键字
 ...
 - extra  : 为一个结构体实现扩展方法; extra Funcs: MyStruct 中的 Funcs 只是扩展方法组的名字, 无实际操作, 与 extra MyStruct 等价
 - impl   : 为一个结构体实现 trait
 - trait  : 同 rust
 ...
 - in : 用于判断一个值或对象是否存在于另一个对象内; 对 Map 判断 key 是否存在
 - let : 定义变量, 必须携带类型; 唯一例外是 for 循环中的迭代变量可以省略类型
 - fn : 定义函数的关键字
 - pub: 声明一个方法/属性/类型/变量/结构体/类/trait/group为公开
 - return: 用于函数内返回值; 返回类型为 None 的函数可省略 return, 隐式返回 None
 - for-in: Python 风格的迭代器协议, 即 `for ele in iterable`; 增强写法 `for (Type ele: iterable)` 是其语法糖 (见 1.4), 其中 Type 可以省略; `impl Trait for Struct` 中的 for 与此无关
...
> 示例中的 `output`、`entry`、`get_last`、`format`、`matches`、`length` 等方法仅为描述性伪代码, 具体由 rt 实现, 不属于语法规范.
