...
| ❌   | 44   | SA 未在 用户调用结构体对象的非 `pub` 字段时报错                                                                                                          | 假阳: `bugs/bug44.wind` 为同文件访问, 按模块可见性 (Rust 语义) 合法; 跨模块读/写/构造/extra 访问私有字段均精确报错 (见 todo-13 用例) |                                                                                                                |
...
| ⬜   | 9    | 决定使用 llvmlite 而不是手搓虚拟机          | 建议型的显式函数编译期展开标记, 思路来自[`Alum`](https://github.com/wayuto/Alum)的`func(pure)`                                                                         |
