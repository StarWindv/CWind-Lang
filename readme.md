...
|      | const-expr                                                                                                   | macro-rules | derive | 函数式过程宏 | 属性宏 | let 定义变量类型自动推断 | 尾返回无需 return | 带值枚举 | CFFI     | 借用检查器 | 生命周期标注 | trait 与 泛型 | 关联类型/常量 | 超 trait | 负 trait | 闭包 | match | `?` | 迭代器糖 | `move` | 异步 | 高层自建类型 | 自举 |
|------|--------------------------------------------------------------------------------------------------------------|-------------|--------|--------------|--------|--------------------------|-------------------|----------|----------|------------|--------------|---------------|---------------|----------|----------|------|-------|-----|----------|--------|------|--------------|------|
| 状态 | ✅                                                                                                           | ✅          | ✅     | ✅           | ✅     | ⬜                       | ✅                | ✅       | ✅       | ✅         | 🚫           | ✅            | ✅            | ✅       | ✅       | ✅   | ✅    | ✅  | ✅       | ⬜     | ⬜   | ⬜           | ⬜   |
| 说明 | 实现路径、覆盖面、限制不同, 见[此处](https://github.com/starwindv/cwind-lang/blob/main/assets/const-expr.md) |             |        |              |        |                          |                   |          | 语法修改 |            | 有 GC        |               |               |          |          |      |       |     |          |        |      |              |      |
